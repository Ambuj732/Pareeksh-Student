        ...
        if (code == 1000) {
          await axios
            ...
            .then(function (res) {
              ...
              console.log("PractcialId::", practical_id);
              // if (typeof practical_id != "undefined") {
              //   let sett = {
              ...
              if (
                ...
                localStorage.setItem(
                  ...
                  JSON.stringify({
                    usercode: response.data.usercode,
                    student_id: response.data.std_id,
                    exam_id: response.data.exam_id,
                    practical_id: response.data.practical_id,
                    assessor_id: response?.data?.id_assessor,
                  })
                );
                sessionStorage.setItem("pkshn_vc_set", JSON.stringify(sett));
                var qstring =
                ...
                // use of qstring to send data in frontend side - you have to seee in backend side okay.
                var qstring =
                  response.data.usercode +
                  "//@spwd++sup@//" +
                  ...
                  "//@spwd++sup@//" +
                  6;
                // query string yanha se start hai setup by btoa
                var parms = window.btoa(qstring);
                navigate("/login-with-passcode/verify-otp?q=" + parms);
                // navigate("/login-with-passcode/?q="+parms);
              } else {
