...
function Question() {
	...
	const [isCurrentTabUsingCamera, setIsCurrentTabUsingCamera] =
		useState(false);
	...
	useEffect(() => {
		if (user && exam && !sendImageIntervalRef.current) {
			sendImageIntervalRef.current = setInterval(
				sendRandomImageHandler,
				60000
			);
		}
	...
	const getTheoryQuestionByNoHandler = async (value, idx) => {
		try {
			if (value && !idx) {
				...
				while (
					locked[currIndex + tempVal - 1] == true &&
					tempVal < 1000
				) {
	...
	const navigateToFinalSubmitPage = (value = 1) => {
		...
		if (Number(currentQuestion?.index) + value >= examInitial?.totalq + 1) {
			...
			localStorage.setItem(
				"ps_totalq",
				JSON.stringify(examInitial?.totalq)
			);
			console.log("Here");
			navigate("/final-submit");
			console.log("Here");
	...
	const checkDeviceInUse = async (
		...
		try {
			...
			if (error.name === "NotReadableError" && !isCurrentTabUsingDevice) {
				Swal.fire(
					`${
						(deviceType == "video" ? "Camera" : "Microphone")
							.charAt(0)
							.toUpperCase() +
						(deviceType == "video" ? "Camera" : "Microphone").slice(
							1
						)
				...
				console.log(
					`An unknown error occurred with ${deviceType}: `,
					error
				);
			}
		} finally {
